...
prad_expr <- read.delim("C:/R_Data/PRAD_Gene_Expression_Data.tsv",
                        header = TRUE,
                        row.names = 1,
                        check.names = FALSE)
...
prad_expr <- data.frame(lapply(prad_expr, as.numeric),
                        row.names = rownames(prad_expr),
                        check.names = FALSE)
...
colnames(prad_expr) <- paste0("PRAD_", colnames(prad_expr))

# ==============================================================================
# 4. Combine the two labelled groups

stopifnot(all(rownames(general_expr) == rownames(prad_expr)))
expr_combined <- cbind(general_expr, prad_expr)
...
group <- c(
  ...
  rep("PRAD_Cluster", ncol(prad_expr))
)

group <- factor(group, levels = c("GeneralCluster", "PRAD_Cluster"))
...
contrast.matrix <- makeContrasts(
  PRAD_vs_General = PRAD_Cluster - GeneralCluster,
  levels = design
)
...
ggplot(volcano_df, aes(x = logFC, y = negLogAdjP, color = regulation)) +
  ...
  labs(
    title = "Volcano Plot: PRAD vs General",
    x = "log2 Fold Change",
    y = "-log10 Adjusted P-value",
    color = "Regulation"
  )

# Right side red genes = upregulated in PRAD
# Left side blue genes = downregulated in PRAD

# ==============================================================================
# 12. Save output files

outdir <- "PRAD_vs_General_limma_trend_results"
...
write.csv(res,
          file.path(outdir, "PRAD_vs_General_full.csv"),
          row.names = FALSE)

write.csv(up_genes,
          file.path(outdir, "PRAD_vs_General_UP.csv"),
          row.names = FALSE)

write.csv(down_genes,
          file.path(outdir, "PRAD_vs_General_DOWN.csv"),
          row.names = FALSE)
# ==============================================================================
# DAVID input files
write.table(up_genes$gene_id,
            file.path(outdir, "DAVID_PRAD_vs_General_UP.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)

write.table(down_genes$gene_id,
            file.path(outdir, "DAVID_PRAD_vs_General_DOWN.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)

write.table(rownames(log_expr),
            file.path(outdir, "DAVID_background_PRAD_vs_General.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
# ==============================================================================
