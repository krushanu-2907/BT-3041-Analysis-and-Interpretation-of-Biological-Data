...
general_expr <- read.delim("C:/R_Data/Central_cluster_tumor_Gene_Expression_Data.tsv",
                           header = TRUE,
                           row.names = 1,
                           check.names = FALSE)

coad_read_expr <- read.delim("C:/R_Data/COAD_READ_Gene_Expression_Data.tsv",
                        header = TRUE,
                        row.names = 1,
                        check.names = FALSE)
...
coad_read_expr <- data.frame(lapply(coad_read_expr, as.numeric),
                        row.names = rownames(coad_read_expr),
                        check.names = FALSE)
...
colnames(coad_read_expr) <- paste0("COAD_READ_", colnames(coad_read_expr))

# ==============================================================================
# 4. Combine the two labelled groups

stopifnot(all(rownames(general_expr) == rownames(coad_read_expr)))
expr_combined <- cbind(general_expr, coad_read_expr)
...
group <- c(
  ...
  rep("COAD_READ_Cluster", ncol(coad_read_expr))
)

group <- factor(group, levels = c("GeneralCluster", "COAD_READ_Cluster"))
...
contrast.matrix <- makeContrasts(
  COAD_READ_vs_General = COAD_READ_Cluster - GeneralCluster,
  levels = design
)
...
ggplot(volcano_df, aes(x = logFC, y = negLogAdjP, color = regulation)) +
  ...
  labs(
    title = "Volcano Plot: COAD + READ vs General",
    x = "log2 Fold Change",
    y = "-log10 Adjusted P-value",
    color = "Regulation"
  )

# Right side red genes = upregulated in COAD + READ
# Left side blue genes = downregulated in COAD + READ

# ==============================================================================
# 12. Save output files

outdir <- "COAD+READ_vs_General_limma_trend_results"
...
write.csv(res,
          file.path(outdir, "COAD+READ_vs_General_full.csv"),
          row.names = FALSE)

write.csv(up_genes,
          file.path(outdir, "COAD+READ_vs_General_UP.csv"),
          row.names = FALSE)

write.csv(down_genes,
          file.path(outdir, "COAD+READ_vs_General_DOWN.csv"),
          row.names = FALSE)
# ==============================================================================
# DAVID input files
write.table(up_genes$gene_id,
            file.path(outdir, "DAVID_COAD+READ_vs_General_UP.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)

write.table(down_genes$gene_id,
            file.path(outdir, "DAVID_COAD+READ_vs_General_DOWN.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)

write.table(rownames(log_expr),
            file.path(outdir, "DAVID_background_COAD+READ_vs_General.txt"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
# ==============================================================================
